...
from DLModels.DKM.utils.utils import get_tuple_transform_ops
from DLModels.DKM.utils.local_correlation import local_correlation
...
class Decoder(nn.Module):
    def __init__(
            self, embedding_decoder, gps, proj, conv_refiner, transformers=None, detach=False, scales="all",
            pos_embeddings=None,
    ):
        super().__init__()
...
class RegressionMatcher(nn.Module):
    ...
    def match(
        ...
        assert not (
                    batched and self.upsample_preds), "Cannot upsample preds if in batchmode (as we don't have access to high res images). You can turn off upsample_preds by model.upsample_preds = False "
