...
class ASpanFormer(nn.Module):
    ...
    def forward(self, data, online_resize=False):
        ...
        [feat_c0, pos_encoding0], [feat_c1, pos_encoding1] = self.pos_encoding(feat_c0,
                                                                               data['pos_scale0']), self.pos_encoding(
    ...
    def resize_input(self, data, train_res, df=32):
        ...
        torch.tensor([w0 / data['image0'].shape[3], h0 / data['image0'].shape[2]])[None], \
            torch.tensor([w1 / data['image1'].shape[3], h1 / data['image1'].shape[2]])[None]
