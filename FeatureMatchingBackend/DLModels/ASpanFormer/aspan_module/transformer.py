...
class messageLayer_gla(nn.Module):
    ...
    def forward(self, x0, x1, flow_feature0, flow_feature1, pos0, pos1, mask0=None, mask1=None, ds0=[4, 4], ds1=[4, 4]):
        ...
        """
        flow0, flow1 = self.decode_flow(flow_feature0, flow_feature1.shape[2:]), self.decode_flow(flow_feature1,
                                                                                                  flow_feature0.shape[
                                                                                                  2:])
...
class LocalFeatureTransformer_Flow(nn.Module):
    ...
    def __init__(self, config):
        ...
        encoder_layer = messageLayer_gla(
            config['d_model'], config['d_flow'], config['d_flow'] + config['d_model'], config['nhead'],
            config['radius_scale'], config['nsample'])
        encoder_layer_last = messageLayer_gla(
            config['d_model'], config['d_flow'], config['d_flow'] + config['d_model'], config['nhead'],
            config['radius_scale'], config['nsample'], update_flow=False)
    ...
    def forward(self, feat0, feat1, pos0, pos1, mask0=None, mask1=None, ds0=[4, 4], ds1=[4, 4]):
        ...
        for layer in self.layers:
            feat0, feat1, flow_feature0, flow_feature1, flow0, flow1 = layer(feat0, feat1, flow_feature0, flow_feature1,
                                                                             pos0, pos1, mask0, mask1, ds0, ds1)
        ...
        feat0, feat1 = feat0.permute(0, 2, 3, 1).view(bs, -1, self.d_model), feat1.permute(0, 2, 3, 1).view(bs, -1,
                                                                                                            self.d_model)
