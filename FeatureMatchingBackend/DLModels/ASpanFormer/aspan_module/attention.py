...
class HierachicalAttention(Module):
    def __init__(self, d_model, nhead, nsample, radius_scale, nlevel=3):
        ...
        sample_offset = torch.tensor([[pos[0] - nsample[1] / 2 + 0.5, pos[1] - nsample[1] / 2 + 0.5] for pos in
                                      product(range(nsample[1]), range(nsample[1]))])  #r^2*2
    ...
    def forward(self, query, key, value, flow, size_q, size_kv, mask0=None, mask1=None, ds0=[4, 4], ds1=[4, 4]):
        ...
        q_list = [F.avg_pool2d(query.view(bs, -1, h0, w0), kernel_size=sub_size, stride=sub_size) for sub_size in
                  sub_sample0]
        k_list = [F.avg_pool2d(key.view(bs, -1, h1, w1), kernel_size=sub_size, stride=sub_size) for sub_size in
                  sub_sample1]
        v_list = [F.avg_pool2d(value.view(bs, -1, h1, w1), kernel_size=sub_size, stride=sub_size) for sub_size in
                  sub_sample1]  #n_level

        offset_list = [F.avg_pool2d(offset.permute(0, 3, 1, 2), kernel_size=sub_size * self.nsample[0],
                                    stride=sub_size * self.nsample[0]).permute(0, 2, 3, 1) / sub_size for sub_size in
                       sub_sample0[1:]]  #n_level-1
        span_list = [F.avg_pool2d(span_scale.permute(0, 3, 1, 2), kernel_size=sub_size * self.nsample[0],
                                  stride=sub_size * self.nsample[0]).permute(0, 2, 3, 1) for sub_size in
                     sub_sample0[1:]]  #n_level-1
        ...
        message_list.append(
            self.fullattention(q_list[0], k_list[0], v_list[0], mask0_flatten, mask1_flatten, self.temp).view(bs,
                                                                                                              self.d_model,
                                                                                                              h0 // ds0[
                                                                                                                  0],
                                                                                                              w0 // ds0[
        ...
        for index in range(1, self.nlevel):
            ...
            q, k, v, sample_pixel, mask_sample = self.partition_token(q, k, v, o, s,
                                                                      mask0)  # B*Head*D*G*N(G*N=H*W for q)
            message_list.append(
                self.group_attention(q, k, v, 1, mask_sample).view(bs, self.d_model, h0 // sub_sample0[index],
    ...
    def partition_token(self, q, k, v, offset, span_scale, maskv):
        ...
        q = q.view(bs, -1, h // self.nsample[0], self.nsample[0], w // self.nsample[0], self.nsample[0]). \
            permute(0, 1, 2, 4, 3, 5).contiguous().view(bs, self.nhead, self.d_model // self.nhead, -1,
                                                        self.nsample[0] ** 2)  #B*head*D*G*N(G*N=H*W for q)
        #sample token
        k = F.grid_sample(k, grid=sample_norm, align_corners=True).view(bs, self.nhead, self.d_model // self.nhead, -1,
                                                    self.nsample[1] ** 2)  #B*head*D*G*r^2
        v = F.grid_sample(v, grid=sample_norm, align_corners=True).view(bs, self.nhead, self.d_model // self.nhead, -1,
                                                    self.nsample[1] ** 2)  #B*head*D*G*r^2
        #import pdb;pdb.set_trace()
        if maskv is not None:
            mask_sample = F.grid_sample(maskv.view(bs, -1, h, w).float(), grid=sample_norm,
                                        mode='nearest', align_corners=True) == 1  #B*1*G*r^2
        else:
            mask_sample = None
