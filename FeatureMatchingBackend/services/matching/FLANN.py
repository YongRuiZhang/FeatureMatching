...
def detection_FLANN(frame1, frame2, kptsMethod):
    if kptsMethod == 'SIFT':
...
def draw_matching(frame1, frame2, kpts1, kpts2, better_matches, kptsMethod, small_text):
    out = cv2.drawMatchesKnn(frame1, kpts1, frame2, kpts2, [better_matches], None)
    H0, _ = frame1.shape
    H1, _ = frame2.shape
    H = max(H0, H1)
    sc = min(H / 640., 2.0)
    ...
    txt_color_bg = (0, 0, 0)
    text = [
        'FLANN_{}'.format(kptsMethod),
        'kpts:{}:{}'.format(len(kpts1), len(kpts2)),
        'Matches: {}'.format(len(better_matches)),
    ]
...
def matching_FLANN_pair(path, img1_path, img2_path, kptsMethod, K, is_save=True, is_process=True, timer=None):
    ...
    if is_save:
        ...
        save_path = os.path.join(save_dir, "FLANN_{}_viz.png".format(kptsMethod))
...
def matching_FLANN_images(path, kptsMethod, K, fix=True, type='多张图片', image_glob=None, skip=1, max_length=1000000,
                          resize=None, fps=1):
